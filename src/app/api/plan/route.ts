  ...
  try {
    ...
    if (recipes.length < 4) {
      return NextResponse.json(
        { error: 'Need at least 4 recipes in the bank to generate a plan' },
        { status: 400 }
      );
    }

    const systemPrompt = buildPlanPrompt(recipes, body.excludeRecipeIds, body.notes);
    ...
    if (!valid || result.selectedRecipeIds.length !== 4) {
